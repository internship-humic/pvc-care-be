...
class PvcScanService extends BaseService {
  ...
  async getById(id, userId, role) {
    ...
    if (role === "Patient") {
      ...
      if (!patientProfile || scan.patient_profile_id !== patientProfile.id) {
        throw this.error.forbidden("You are not authorized to access this scan");
      }
    } else if (role === "Doctor") {
      const doctorProfile = await this.db.doctorProfile.findUnique({
        where: { user_id: userId }
      });
      if (!doctorProfile || scan.doctor_profile_id !== doctorProfile.id) {
        throw this.error.forbidden("You are not authorized to access this scan");
      }
    }
